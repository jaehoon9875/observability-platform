...
Operator는 `observability-platform` 네임스페이스에 배포되며, InnoDBCluster 리소스를 감시합니다.
...
```bash
kubectl get deployment -n observability-platform mysql-operator
...
# 클러스터 상태 확인 (5~10분 소요)
kubectl get innodbcluster -n observability-platform --watch
...
kubectl describe innodbcluster mysql-cluster -n observability-platform

# Operator 로그 확인
kubectl logs -n observability-platform deployment/mysql-operator
```
...
# MySQL Shell 진입 후
MySQL JS> \connect root@mysql-cluster.observability-platform.svc.cluster.local
...
kubectl port-forward service/mysql-cluster -n observability-platform 3306:6446
...
```
호스트: mysql-cluster.observability-platform.svc.cluster.local
