...
`infra/manifests/mysql/mysql-cluster.yaml`을 적용하여 MySQL InnoDB Cluster를 생성합니다.
MySQL Operator가 CRD를 읽고 StatefulSet, Service 등을 자동으로 생성합니다.

> 클러스터 정의 파일: [`infra/manifests/mysql/mysql-cluster.yaml`](../infra/manifests/mysql/mysql-cluster.yaml)
...
```bash
kubectl apply -f infra/manifests/mysql/mysql-cluster.yaml
